...
ConVar g_APIGOEXANIMOURLCvar;
char g_APIGOEXANIMOURL[128];

#define LOGO_DIR "resource/flash/econ/tournaments/teams"

// clang-format off
public Plugin myinfo = {
  name = "Get5 GOEXANIMO Web API Integration",
  author = "splewis/gelzis",
  description = "Records match stats to a besl api",
  version = PLUGIN_VERSION,
  url = "https://github.com/gelzis/get5"
};
// clang-format on
...
public void OnPluginStart() {
  ...
  g_APIGOEXANIMOURLCvar = CreateConVar("get5_goexanimo_api_url", "", "URL the goexanimo api is hosted at");

  HookConVarChange(g_APIGOEXANIMOURLCvar, ApiInfoChanged);
...
public void ApiInfoChanged(ConVar convar, const char[] oldValue, const char[] newValue) {
  ...
  g_APIGOEXANIMOURLCvar.GetString(g_APIGOEXANIMOURL, sizeof(g_APIGOEXANIMOURL));
  ...
  LogDebug("get5_goexanimo_api_url now set to %s", g_APIGOEXANIMOURL);
...
static Handle CreateRequestGoexanimo(EHTTPMethod httpMethod, const char[] apiMethod, any:...) {
  char url[1024];
  Format(url, sizeof(url), "%s%s", g_APIGOEXANIMOURL, apiMethod);
...
public void Get5_OnDemoFinished(const char[] fileName) {
  ...
  Handle req = CreateRequestGoexanimo(k_EHTTPMethodPOST, "demo");
  if (req != INVALID_HANDLE) {
    AddStringParam(req, "matchId", matchId);
    AddIntParam(req, "mapNumber", mapNumber);
    AddStringParam(req, "fileName", fileName);
    SteamWorks_SendHTTPRequest(req);
